...
Query.prototype.joins = function(joins) {
  ...
  joins.forEach(function(join) {
    ...
    alias = utils.escapeTable(join.alias.toLowerCase() + '_' + tableName);

    // Escape Table Name
    tableName = utils.escapeTable(tableName);

    // Build Full Key Names
    foreignKey = alias + '.' + utils.escapeTable(join.childKey);
    ...
    joins.forEach(function(otherJoin) {
      if(otherJoin.child === join.parent) {
        parentKey = utils.escapeTable(otherJoin.alias.toLowerCase() + '_' + otherJoin.child.toLowerCase());
        parentKey += '.' + utils.escapeTable(join.parentKey);
      }
    });
