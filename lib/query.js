...
Query.prototype.operators = function() {
  ...
  var sql = {
    and: function(key, options, comparator) {
      processCriteria.call(self, key, options, '=', true, false);
      self._query += (comparator || ' AND ');
    },

    like: function(parent, key, options, comparator) {
      processCriteria.call(self, parent, options[key][parent], 'ILIKE', true, true);
      self._query += (comparator || ' AND ');
    },
...
function processCriteria(parent, value, combinator, caseSensitive, cast) {
  var self = this;
  ...
  if(typeof value === 'object' && value !== null) {
    ...
    // CAST to TEXT if case sensitive
    if(cast) parent = 'CAST(' + parent + ' AS TEXT)';
