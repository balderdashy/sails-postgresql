...
function processCriteria(table, parent, value, combinator, caseSensitive) {
  ...
  function expandCriteria(obj) {
    ...
    Object.keys(obj).forEach(function(key) {
      ...
      if(self._schema[parent] && self._schema[parent].type === 'text[]'){
        prepareCriterion.call(self, key, obj[key], parent);
        self._query += 'ANY('+_param + ') ';
      }
      else{
        self._query += _param + ' ';
        prepareCriterion.call(self, key, obj[key],parent);
...
function prepareCriterion(key, value,parent) {
  var str;
  ...
  switch(key) {
    ...
    case 'like':
      this._values.push(value);
      str = 'ILIKE ' + '$' + this._paramCount;this
      break;

    case 'contains':
        if(this._schema[parent] && this._schema[parent].type === 'text[]'){
            //this is a postgres array, WHERE 'test' = ANY(tags)
            this._values.push(value);
            str =  '$' + this._paramCount +' = ';
        }
        else{
