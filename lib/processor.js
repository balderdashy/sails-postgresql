 ...
 */
 ...
 Processor.prototype.castValue = function(table, key, value, attributes) {
  ...
  // Get Waterline identity based on the table name
  var identity = _.find(_.values(this.schema), {tableName: table}).identity;

  // Transform the key into an attribute name if it's a custom column name
  var attr = _.reduce(this.schema[identity].attributes, function(memo, val, _attr) {
    // If we find an attribute that has the key as a custom column name,
    // return the name of that attribute
    if (val.columnName == key) {
      return _attr;
    } else {
      return memo;
    }
