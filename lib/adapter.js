...
module.exports = (function() {
  ...
  var adapter = {
    ...
    registerConnection: function registerConnection(connectionConfig, models, cb) {
      ...
      if (!connectionConfig.identity) {
        return cb(Errors.IdentityMissing);
      }

      if (datastores[connectionConfig.identity]) {
      ...
      if (!_.has(connectionConfig, 'url')) {
        ...
        if (connectionConfig.user && connectionConfig.password) {
          url += connectionConfig.user + ':' + connectionConfig.password + '@';
        }

        url += connectionConfig.host + ':' + connectionConfig.port + '/' + connectionConfig.database;
        connectionConfig.url = url;
      }

      // Create a manager to handle this connection config
      pg.createManager({
        connectionString: connectionConfig.url,
        onUnexpectedFailure: function onUnexpectedFailure(err) {
      ...
      .exec(function createManagerCb(err, report) {
        ...
        // connectionOverrides[connectionConfig.identity] = {};

        // Build up a database schema for this connection that can be used
        // throughout the adapter
        var dbSchema = {};

        _.each(models, function buildSchema(val, key) {
          ...
          if (!_.includes(val.connection, connectionConfig.identity)) {
            return;
          }

          // If this collection has a schema name set, make sure that it's passed
          // through to the options for waterline-sequel.
          // if (_.has(val, 'meta') && _.has(val.meta, 'schemaName')) {
          //   sqlOptions.schemaName[_schema.tableName] = val.meta.schemaName;
          // } else {
        ...
        datastores[connectionConfig.identity] = {
    ...
    //  ╝╚╝╩ ╩ ╩ ╩ ╚╝ ╚═╝  ╚═╝╚╚═╝╚═╝╩╚═ ╩
    //
    query: function query(connectionName, table, query, data, cb) {
    ...
    //   ║║║╣ ╚═╗║  ╠╦╝║╠╩╗║╣    ║ ╠═╣╠╩╗║  ║╣
    //  ═╩╝╚═╝╚═╝╚═╝╩╚═╩╚═╝╚═╝   ╩ ╩ ╩╚═╝╩═╝╚═╝
    describe: function describe(datastoreName, table, cb) {
      spawnConnection(datastoreName, function describeQuery(connection, cb) {
        var datastoreConfig = datastores[datastoreName];
        var model = datastoreConfig.models[table];
        var tableName = table;
        var schemaName = getSchema(datastoreName, table);
        ...
        async.auto({
          ...
          // Run Info Query
          infoQuery: function infoQuery(next) {
            pg.sendNativeQuery({
              connection: connection,
              nativeQuery: query
            })
            .exec(function execCb(err, data) {
              if (err) {
                return next(err);
              }
          ...
          // Get Auto-Incrementing Sequences
          sequences: function sequences(next) {
            pg.sendNativeQuery({
              connection: connection,
              nativeQuery: autoIncrementQuery
            })
            .exec(function execCb(err, data) {
              if (err) {
                return next(err);
          ...
          // Get Indexed Columns
          indicies: function indicies(next) {
            pg.sendNativeQuery({
              connection: connection,
              nativeQuery: indiciesQuery
            })
            .exec(function execCb(err, data) {
              if (err) {
                return next(err);
        ...
        function processResults(err, results) {
          ...
          var schemaResults = results.infoQuery;

          // Add autoIncrement flag to schema
          _.each(results.sequences.rows, function processSequenceRow(row) {
            if (row.related_table !== table) {
              return;
            }

            // Look through query results and see if related_column exists
            _.each(schemaResults.rows, function extendColumn(column) {
          ...
          _.each(results.indicies.rows, function processIndexRow(column) {
          ...
          // Normalize Schema
          var normalizedSchema = utils.normalizeSchema(schemaResults.rows);

          // Set Internal Schema Mapping
          model.dbSchema = normalizedSchema;
    ...
    //  ═╩╝╚═╝╚  ╩╝╚╝╚═╝   ╩ ╩ ╩╚═╝╩═╝╚═╝
    //
    define: function define(datastoreName, table, definition, cb) {
      // Create a describe method to run after the define.
      // Ensures the define connection is properly closed.
      var describe = function describe(err) {
        if (err) {
          return cb(err);
        }

        // Describe (sets schema)
        adapter.describe(datastoreName, table.replace(/["']/g, ''), cb);
      };

      var datastoreConfig = datastores[datastoreName];

      spawnConnection(datastoreName, function defineQuery(connection, cb) {
        // Get the schema name if any
        var schemaName = getSchema(datastoreName, table);

        // If we're being told NOT to create schemas, then skip right to
        // creating the table.
        if (datastoreConfig.config.createSchemas === false) {
          return _define();
        }
        ...
        adapter.createSchema(datastoreName, table, schemaName, _define);

        // Define function builds the table
        var _define = function _define(errCreatingSchema) {
          if (errCreatingSchema) {
            return cb(handleQueryError(errCreatingSchema));
          }
          ...
          .exec(function execCb(err) {
            if (err) {
              return cb(err);
            }

            // Build Indexes
            var buildIndex = function buildIndex(name, nextIndex) {
    ...
    //  ╚═╝╩╚═╚═╝╩ ╩ ╩ ╚═╝  ╚═╝╚═╝╩ ╩╚═╝╩ ╩╩ ╩
    //
    createSchema: function createSchema(datastoreName, table, schemaName, cb) {
      if ('function' == typeof schemaName) {
        cb = schemaName;
        schemaName = getSchema(datastoreName, table);
      }
      ...
      spawnConnection(datastoreName, function spawnConnection(connection, cb) {
        // Run the query
        pg.sendNativeQuery({
          connection: connection,
          nativeQuery: query
    ...
    //  ═╩╝╩╚═╚═╝╩     ╩ ╩ ╩╚═╝╩═╝╚═╝
    //
    drop: function drop(datastoreName, table, relations, cb) {
      ...
      spawnConnection(datastoreName, function spawnConnection(connection, cb) {
        // Drop any relations
        var dropTable = function dropTable(item, next) {
          var tableName = utils.escapeName(item, getSchema(datastoreName, item));
        ...
        async.eachSeries(relations, dropTable, function processRelation(err) {
          if (err) {
            return cb(err);
          }
    ...
    //  ╩ ╩═╩╝═╩╝  ╩ ╩ ╩  ╩ ╩╚═╩╚═╝╚═╝ ╩ ╚═╝
    //
    addAttribute: function addAttribute(datastoreName, table, attrName, attrDef, cb) {
      spawnConnection(datastoreName, function spawnConnection(connection, cb) {
        // Escape Table Name
        table = utils.escapeName(table, getSchema(datastoreName, table));
    ...
    //  ╩╚═╚═╝╩ ╩╚═╝ ╚╝ ╚═╝  ╩ ╩ ╩  ╩ ╩╚═╩╚═╝╚═╝ ╩ ╚═╝
    //
    removeAttribute: function removeAttribute(datastoreName, table, attrName, cb) {
      spawnConnection(datastoreName, function spawnConnection(connection, cb) {
        // Escape Table Name
        table = utils.escapeName(table, getSchema(datastoreName, table));
    ...
    create: function create(datastoreName, tableName, data, cb) {
    ...
    //  ╚═╝╚═╝╩═╝╚═╝╚═╝ ╩   └─┘└└─┘└─┘┴└─ ┴
    // Select Query Logic
    find: function find(datastoreName, tableName, criteria, cb) {
      var datastore = datastores[datastoreName];
      Helpers.select({
        datastore: datastore,
        tableName: tableName,
    ...
    update: function update(datastoreName, tableName, criteria, values, cb) {
      var datastore = datastores[datastoreName];
      Helpers.update({
        datastore: datastore,
        tableName: tableName,
    ...
    destroy: function destroy(datastoreName, tableName, criteria, cb) {
      var datastore = datastores[datastoreName];
      Helpers.destroy({
        datastore: datastore,
        tableName: tableName,
