...
module.exports = (function() {
  ...
  var adapter = {
    ...
    stream: function(connectionName, table, options, stream) {
      spawnConnection(connectionName, function __STREAM__(client, cb) {
        ...
        var streamQuery = new QueryStream(_query.query[0], _query.values[0]);
        var dbStream = client.query(streamQuery);

        // Can stream row results back 1 at a time
        dbStream.on('data', function(row) {
          stream.write(row);
        });
        //
