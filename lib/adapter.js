...
module.exports = (function() {
  ...
  var adapter = {
    ...
    define: function(connectionName, table, definition, cb) {
      ...
      spawnConnection(connectionName, function __DEFINE__(client, cb) {
        ...
        var schemaName = collection.meta && collection.meta.schemaName
                        ...
                        : '';
        table = schemaName + utils.escapeName(table);
      ...
      spawnConnection(connectionName, function __DROP__(client, cb) {
        ...
        function dropTable(item, next) {
          var schemaName = collection.meta && collection.meta.schemaName
                          ? utils.escapeName(collection.meta.schemaName) + '.'
                          : '';
          var table = schemaName + utils.escapeName(item);
    ...
    // Bulk load using streaming option
    createEachStream: function(connectionName, table, records, cb) {
      spawnConnection(connectionName, function __CREATE_EACH_STREAM__(client, cb) {
        var connectionObject = connections[connectionName];
        var collection = connectionObject.collections[table];
        ...
        var schemaName = collection.meta && collection.meta.schemaName
                         ...
                         : '';
        var tableName = schemaName + utils.escapeName(table);
        var colNames = Object.keys(collection.schema).map(function(key) { return utils.escapeName(key);}).join(',');
        var query = 'COPY ' + tableName + ' (' + colNames + ') FROM STDIN WITH CSV';
        var stream = client.copyFrom(query);
        ...
        records.forEach(function(item) {
          var insdata = [];
          Object.keys(collection.schema).forEach(function(key) {
            if(_.has(item, key)) {
              insdata.push(utils.prepareValue(item[key]));
            } else {
              insdata.push(null);
            }
          });
          stream.write(insdata.join(',')+'\n');
        });
        stream.end();
      }, cb);
    },
