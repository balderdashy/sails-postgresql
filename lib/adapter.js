...
module.exports = (function() {
  ...
  var adapter = {
    ...
    registerConnection: function registerConnection(connection, collections, cb) {
      ...
      _.each(collections, function buildSchema(val, key) {
        ...
        if (!_.includes(val.connection, connection.identity)) {
          return;
        }
      ...
      async.map(Object.keys(collections), function describe(colName, cb) {
    ...
    //   ║║║╣ ╚═╗║  ╠╦╝║╠╩╗║╣    ║ ╠═╣╠╩╗║  ║╣
    //  ═╩╝╚═╝╚═╝╚═╝╩╚═╩╚═╝╚═╝   ╩ ╩ ╩╚═╝╩═╝╚═╝
    describe: function describe(connectionName, table, cb) {
      spawnConnection(connectionName, function describeQuery(connection, cb) {
    ...
    //  ═╩╝╚═╝╚  ╩╝╚╝╚═╝   ╩ ╩ ╩╚═╝╩═╝╚═╝
    //
    define: function define(connectionName, table, definition, cb) {
      ...
      spawnConnection(connectionName, function defineQuery(connection, cb) {
    ...
    //  ╚═╝╩╚═╚═╝╩ ╩ ╩ ╚═╝  ╚═╝╚═╝╩ ╩╚═╝╩ ╩╩ ╩
    //
    createSchema: function createSchema(connectionName, table, schemaName, cb) {
      ...
      spawnConnection(connectionName, function spawnConnection(connection, cb) {
        // Run the query
        pg.sendNativeQuery({
          connection: connection,
          nativeQuery: query
    ...
    //  ═╩╝╩╚═╚═╝╩     ╩ ╩ ╩╚═╝╩═╝╚═╝
    //
    drop: function drop(connectionName, table, relations, cb) {
    ...
    //  ╩ ╩═╩╝═╩╝  ╩ ╩ ╩  ╩ ╩╚═╩╚═╝╚═╝ ╩ ╚═╝
    //
    addAttribute: function addAttribute(connectionName, table, attrName, attrDef, cb) {
      spawnConnection(connectionName, function spawnConnection(connection, cb) {
        // Escape Table Name
    ...
    //  ╩╚═╚═╝╩ ╩╚═╝ ╚╝ ╚═╝  ╩ ╩ ╩  ╩ ╩╚═╩╚═╝╚═╝ ╩ ╚═╝
    //
    removeAttribute: function removeAttribute(connectionName, table, attrName, cb) {
      spawnConnection(connectionName, function spawnConnection(connection, cb) {
        // Escape Table Name
        table = utils.escapeName(table, getSchema(connectionName, table));
  ...
  var spawnConnection = function spawnConnection(connectionName, logic, cb) {
    var connectionObject = connections[connectionName];
    if (!connectionObject) {
  ...
  var getCollection = function getCollection(connectionName, table) {
