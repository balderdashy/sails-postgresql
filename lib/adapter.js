  ...
  function spawnConnection(connectionName, logic, cb) {
    ...
    function after(err, client, done) {
      if(err) {
        ...
        console.error('');
        console.error('Error creating a connection to Postgresql.');
        console.error('');
        ...
        if (tooManyConnections) {
          ...
          'you have your `poolSize` set as something < 20.',
          'The default `poolSize` is 10.');
          console.error(
          '    To override the default `poolSize`, specify a `poolSize` property on the relevant Postgresql',
          '"connection" config object. If you\'re using Sails, this is generally located in `config/connections.js`,',
