...
utils.buildSelectStatement = function(criteria, table, attributes, schema) {
    ...
    if(criteria.groupBy) {
      if(criteria.groupBy instanceof Array) {
        criteria.groupBy.forEach(function(opt) {
          query += utils.escapeTable(table) + '.' + utils.escapeTable(opt) + ', ';
        });

      } else {
        query += utils.escapeTable(table) + '.' + utils.escapeTable(criteria.groupBy) + ', ';
      }
    }
    ...
    if (criteria.sum) {
      if(criteria.sum instanceof Array) {
        criteria.sum.forEach(function(opt){
          query += 'CAST(SUM(' + utils.escapeTable(table) + '.' + utils.escapeTable(opt) + ') AS float) AS ' + opt + ', ';
        });

      } else {
        query += 'CAST(SUM(' + utils.escapeTable(table) + '.' + utils.escapeTable(criteria.sum) + ') AS float) AS ' + criteria.sum + ', ';
      }
    }
    ...
    if (criteria.average) {
      if(criteria.average instanceof Array) {
        criteria.average.forEach(function(opt){
          query += 'CAST(AVG(' + utils.escapeTable(table) + '.' + utils.escapeTable(opt) + ') AS float) AS ' + opt + ', ';
        });

      } else {
        query += 'CAST(AVG(' + utils.escapeTable(table) + '.' + utils.escapeTable(criteria.average) + ') AS float) AS ' + criteria.average + ', ';
      }
    }
    ...
    if (criteria.max) {
      if(criteria.max instanceof Array) {
        criteria.max.forEach(function(opt){
          query += 'MAX(' + utils.escapeTable(table) + '.' + utils.escapeTable(opt) + ') AS ' + opt + ', ';
        });

      } else {
        query += 'MAX(' + utils.escapeTable(table) + '.' + utils.escapeTable(criteria.max) + ') AS ' + criteria.max + ', ';
      }
    }
    ...
    if (criteria.min) {
      if(criteria.min instanceof Array) {
        criteria.min.forEach(function(opt){
          query += 'MIN(' + utils.escapeTable(table) + '.' + utils.escapeTable(opt) + ') AS ' + opt + ', ';
        });

      } else {
        query += 'MIN(' + utils.escapeTable(table) + '.' + utils.escapeTable(criteria.min) + ') AS ' + criteria.min + ', ';
      }
    }
  ...
  joinSelectKeys.forEach(function(select) {
    ...
    if(select.model) {
      return query += utils.escapeTable(alias) + '.' + utils.escapeTable(select.key) + ' AS ' +
                      utils.escapeTable(select.parentKey + '__' + select.key) + ', ';
    }
    ...
    if(select.junctionTable) {
      return query += utils.escapeTable(alias) + '.' + utils.escapeTable(select.key) + ' AS ' +
                  utils.escapeTable(select.alias + '_' + select.child + '__' + select.key) + ', ';
    }
