...
utils.buildSelectStatement = function(criteria, table, count) {
  ...
  if(criteria.groupBy || criteria.sum || criteria.average || criteria.min || criteria.max) {
    ...
    if(criteria.groupBy) {
      if(criteria.groupBy instanceof Array) {
        criteria.groupBy.forEach(function(opt){
          query += escapeName(opt) + ', ';
        });

      } else {
        query += escapeName(criteria.groupBy) + ', ';
      }
    }
    ...
    if (criteria.sum) {
      if(criteria.sum instanceof Array) {
        ...
        criteria.sum = escapeName(criteria.sum);
    ...
    if (criteria.average) {
      if(criteria.average instanceof Array) {
        ...
        criteria.average = escapeName(criteria.average);
    ...
    if (criteria.max) {
      if(criteria.max instanceof Array) {
        ...
        criteria.max = escapeName(criteria.max);
    ...
    if (criteria.min) {
      if(criteria.min instanceof Array) {
        ...
        criteria.min = escapeName(criteria.min);
