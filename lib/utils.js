...
utils.buildSelectStatement = function(criteria, table, attributes, schema) {
  ...
  if(criteria.groupBy || criteria.sum || criteria.average || criteria.min || criteria.max) {
    ...
    if(criteria.groupBy) {
      if(criteria.groupBy instanceof Array) {
        criteria.groupBy.forEach(function(opt) {
          query += utils.escapeName(table) + '.' + utils.escapeName(opt) + ', ';
        });

      } else {
        query += utils.escapeName(table) + '.' + utils.escapeName(criteria.groupBy) + ', ';
      }
    }
    ...
    if (criteria.sum) {
      if(criteria.sum instanceof Array) {
        criteria.sum.forEach(function(opt) {
          query += 'CAST(SUM(' + utils.escapeName(table) + '.' + utils.escapeName(opt) + ') AS float) AS ' + opt + ', ';
        });

      } else {
        query += 'CAST(SUM(' + utils.escapeName(table) + '.' + utils.escapeName(criteria.sum) + ') AS float) AS ' + criteria.sum + ', ';
      }
    }
    ...
    if (criteria.average) {
      if(criteria.average instanceof Array) {
        criteria.average.forEach(function(opt){
          query += 'CAST(AVG(' + utils.escapeName(table) + '.' + utils.escapeName(opt) + ') AS float) AS ' + opt + ', ';
        });

      } else {
        query += 'CAST(AVG(' + utils.escapeName(table) + '.' + utils.escapeName(criteria.average) + ') AS float) AS ' + criteria.average + ', ';
      }
    }
    ...
    if (criteria.max) {
      if(criteria.max instanceof Array) {
        criteria.max.forEach(function(opt){
          query += 'MAX(' + utils.escapeName(table) + '.' + utils.escapeName(opt) + ') AS ' + opt + ', ';
        });

      } else {
        query += 'MAX(' + utils.escapeName(table) + '.' + utils.escapeName(criteria.max) + ') AS ' + criteria.max + ', ';
      }
    }
    ...
    if (criteria.min) {
      if(criteria.min instanceof Array) {
        criteria.min.forEach(function(opt){
          query += 'MIN(' + utils.escapeName(table) + '.' + utils.escapeName(opt) + ') AS ' + opt + ', ';
        });

      } else {
        query += 'MIN(' + utils.escapeName(table) + '.' + utils.escapeName(criteria.min) + ') AS ' + criteria.min + ', ';
      }
    }
    ...
    // Add FROM clause
    return query += 'FROM ' + utils.escapeName(table) + ' ';
  }
  ...
  query = query.slice(0, -2) + ' FROM ' + utils.escapeName(table) + ' ';
